...
from models.best_params.LightGBM_saved_params import HYPERPARAMETERS
...
def objective_lgb(trial, X_train, y_train):
    ...
    params = {
        ...
        'num_leaves': trial.suggest_int('num_leaves', 2, 26, log=True),
...
def get_best_params(target):
    with open(f'./models/best_params/LightGBM_best_params.pkl', 'rb') as f:
...
def lgb_predict_evaluate(best_params, X_train, X_test, y_train, y_test, X_all):
    # Fit model
    opt_model = LGBMRegressor(**best_params, verbose=-1, n_jobs=N_JOBS)
...
def feature_prune_rfecv(best_params, X_train, X_test, y_train, y_test, rmse_train, rmse_test, mae_test):
    ...
    return lgbm_reg_prune, optimal_features
...
if __name__ == '__main__':
    ...
    for target in targets:
        ...
        opt_model, all_predictions, rmse_train, rmse_crossval, rmse_test, mae_test, NSE_test, KGE_test = lgb_predict_evaluate(best_params, X_train, X_test, y_train, y_test, X_all)
        ...
        opt_model = LGBMRegressor(**best_params, verbose=-1)  # type: ignore

        with open(f'./visualization/models/LightGBM/LightGBM_model_{target}.pkl', 'wb') as file:
            pickle.dump(opt_model, file)

        if prune:
            lgb_reg_prune, optimal_features = feature_prune_rfecv(best_params, X_train, X_test, y_train, y_test, rmse_train, rmse_test, mae_test)
    ...
    if tuning:
        with open('./models/best_params/LightGBM_best_params.pkl', 'wb') as file:
            pickle.dump(params_dict, file)

        df_results.to_csv(f"./models/output/LightGBM_tuning_{n_trials}.csv", index=False)
    
    else:
        df_results.to_csv("./models/output/LightGBM_results_inference.csv", index=False)
