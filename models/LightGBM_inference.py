...
if __name__ == '__main__':
    ...
    for target in Response_variables:
        ...
        all_predictions, rmse_train, rmse_mean_crossval, rmse_crossval_folds, rmse_test, mae_test, NSE_test, KGE_test = lgb_predict_evaluate(best_params, X_train, X_test, y_train, y_test, X_all)
    ...
    with open(f"./inference_output/LightGBM_inference.txt", "w") as file:
        file.writelines(df_results.to_string(index=False))
        file.write(f"\n\nTotal elapsed time: {elapsed_time:.4f} seconds")

    print(f"Output saved to LightGBM_inference.txt")