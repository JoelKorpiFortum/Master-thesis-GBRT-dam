...
def objective_lgb(trial, X_train, y_train, penalty_factor):
    ...
    params = {
            ...
            'verbose': -1,
            'random_state': 42,
            'boosting_type': 'gbdt',  # skip DART, takes too long for what its worth
            ...
            'reg_alpha': trial.suggest_float('reg_alpha', 0, 10, step=0.01),    # L1 regularization
            'reg_lambda': trial.suggest_float('reg_lambda', 0, 10, step=0.01),  # L2 regularization
...
if __name__ == '__main__':
    ...
    for target in Response_variables:
        ...
        all_predictions, rmse_train, rmse_crossval, rmse_test, mae_test, NSE_test, KGE_test = lgb_predict_evaluate(best_params, X_train, X_test, y_train, y_test, X_all, penalty_factor)
    ...
    with open(f"./tuning_output/LightGBM/LightGBM_tuning_31_3_pen_{penalty_factor*10:02.0f}.txt", "w") as file:
        file.writelines(df_results.to_string(index=False))
        file.write(f"\n\nTotal elapsed time: {elapsed_time:.4f} seconds")

    print(f"Output saved to LightGBM_tuning_31_3_pen_{penalty_factor*10:02.0f}.txt")