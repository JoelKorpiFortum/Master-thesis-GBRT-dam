...
if __name__ == '__main__':
    ...
    for target in Response_variables:
        ...
        all_predictions, rmse_train, rmse_crossval, rmse_test, mae_test, NSE_test, KGE_test = gbrt_predict_evaluate(best_params, X_train, X_test, y_train, y_test, X_all, penalty_factor)
    ...
    with open(f"./tuning_output/GBRT/GBRT_tuning_31_3_pen_{penalty_factor*10:02.0f}.txt", "w") as file:
        file.writelines(df_results.to_string(index=False))
        file.write(f"\n\nTotal elapsed time: {elapsed_time:.4f} seconds")

    print(f"Output saved to GBRT_tuning_31_3_pen_{penalty_factor*10:02.0f}.txt")